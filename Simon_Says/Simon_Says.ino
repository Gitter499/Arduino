...
long timeLimit = 4000; //time limit to hit a button
    ...
    { //loop until the player presses a button or the time limit is up (the time limit check is in an if statement)
      ...
      { //if a button is pressed... (4 means that no button is pressed)
        ...
        else
        {
          ...
          delay(100);     //if the button doesn't match the button in the sequence
...
void loseSequence()
{
  ...
  Serial.println("HIGHSCORE: " + (String)old_highscore);
